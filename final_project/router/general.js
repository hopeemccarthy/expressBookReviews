...
function getBookByAuthorPromise(author) {
    return new Promise((resolve, reject) => {
        const authorBooks = Object.values(books).filter((b) => b.author === author);
        resolve(authorBooks);
    })
}
...
public_users.get('/author/:author',function (req, res) {
    ...
    getBookByAuthorPromise(author).then((response) => res.send(JSON.stringify(response)));    
});

// Get all books based on title
public_users.get('/title/:title',function (req, res) {
    const title = req.params.title.split('+').join(' ');
    getBookByTitlePromise(title).then((response) => res.send(JSON.stringify(response)));    
});
